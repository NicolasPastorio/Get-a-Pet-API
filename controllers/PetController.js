...
module.exports = class PetController {
    ...
    static async create(req, res){
        ...
        if(!age){
            res.status(244).json({ message: "A idade é obrigatória!" });
            return;
        }
        if(!weight){
            res.status(244).json({ message: "O peso é obrigatório!" });
            return;
        }
        if(!color){
            res.status(244).json({ message: "A cor é obrigatória!" });
            return;
        }
    ...
    static async updatePet(req, res){
        ...
        if(!age){
            res.status(422).json({ messge: "A idade é obrigatória!" });
            return;
        }else{
            updatedData.age = age;
        }
    ...
    static async schedule(req, res){
        ...
        res.status(200).json({ message: `A visita foi agendada com suceso, entre em contato com ${pet.user.name} pelo telefone ${pet.user.phone}`});
    }
